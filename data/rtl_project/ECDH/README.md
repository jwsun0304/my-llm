...
검증했다. AXI-Stream 인터페이스를 사용해 다른 가속기(코드 상 ML-KEM-512 관련 언급이 있어,
PQC 가속기와 함께 쓰는 시스템의 일부로 추정)와 연동 가능하도록 설계됨.
...
| `PQC_KEM.c` | 호스트(임베디드 리눅스) 측 C 드라이버. Vivado AXI DMA로 가속기를 제어하며, 코드 상 ML-KEM-512(PQC) 관련 처리도 함께 있어 PQC 가속기와 연동되는 상위 제어 코드로 추정 |
...
---
*이 문서는 소스 코드를 읽고 자동 생성한 초안입니다. `curve25519.v`의 출처, `PQC_KEM.c`와의 실제 연동 관계 등
